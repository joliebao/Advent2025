...
public class Day2 {
    ...
    // part 2
    private void findInvalid2(String id) {
        // split into each ID range
        String[] IDs = id.split(",");
        for (String range : IDs) {
            String first = range.substring(0, range.indexOf("-"));
    ...
    private boolean repeatedValue(long numberInt){
        ...
        String repVal = num.charAt(0)+"";

        for (int index = 0; index < num.length()/2; index++) { // while less than half way repeating
            num = num.replaceAll(repVal, "");
            if (num.isEmpty()){
                return true;
            } else {
                num = String.valueOf(numberInt);
                repVal += num.charAt(index+1);
            }
        }
        return (num.isEmpty());
    }
